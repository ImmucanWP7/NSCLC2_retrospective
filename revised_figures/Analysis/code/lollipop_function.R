...
lollipop_plot <- function(data, condition, var_A, var_B, order = TRUE, order_vec, col_A, col_B) {
  ...
  if(order == TRUE){
    ...
    volcano_df <- volcano_df %>%
      mutate(name = factor(name, levels = order_vec))  # order for plotting
  }
  ...
  p2 <- ggplot(volcano_df, aes(x = delta_z, y = name)) +
    ...
    geom_point(
      ...
      aes(x = delta_z, y = name, size = -log10(pvalue), shape = "FDR < 0.1"),
      color = "black", fill = NA, stroke = 1
    ) +
    scale_color_manual(values = setNames(c(col_A, col_B), c(dir_A, dir_B))) +
    scale_shape_manual(name = NULL, values = c("FDR < 0.1" = 21)) +
    geom_vline(xintercept = 0, linetype = "dashed", color = "grey40") +
    labs(x = paste0("Δ z-score (", var_A, " - ", var_B, ")"), y = NULL,
         color = "Direction", size = "-log10(p-value)") +
    theme_minimal() +
    theme(
